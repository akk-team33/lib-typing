...
enum TypeVariant {
    ...
    private final BiFunction<Type, Map<String, Stage>, Stage> mapping;

    TypeVariant(final Predicate<Type> matching, final BiFunction<Type, Map<String, Stage>, Stage> mapping) {
        this.matching = matching;
        this.mapping = mapping;
    }

    static Stage toStage(final Type type, final Map<String, Stage> parameters) {
        return Stream.of(TypeVariant.values())
                .filter(typeType -> typeType.matching.test(type)).findAny()
                .map(typeType -> typeType.mapping.apply(type, parameters))
                .orElseThrow(() -> new IllegalArgumentException(
                        "Unknown variant of java.lang.reflect.Type: " + type.getClass()));
    }
}
